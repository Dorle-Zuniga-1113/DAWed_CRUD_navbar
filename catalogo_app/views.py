...
def registrarCatalogo(request):
    ...
    nuevo = request.POST["txtnew"]
    
#cambiar los redirect por variable de inicio
    guardarcatalogo=Catalogo.objects.create(codigo=codigo,nombre=nombre,fecha_inicio=fecha_inicio,fecha_fin=fecha_fin,promocion=promocion,coleccion=coleccion,nuevo=nuevo)
...
def editarCatalogo(request):
    ...
    try:
        ...
        loscata.nuevo = request.POST["txtnew"]
        loscata.save()
